...
df['tweets'] = df['tweets'].apply(lambda x: preprocess_text(x,use_lemmatization=False, remove_stop_words=False, use_stemming=True))
#print(df.head)
...
#print(f'F1 Score: {f1}')
def RandomForestOutput(string):
    if pd.isna(string):
        string = 'NaN'
    string = preprocess_text(string,use_lemmatization=False, remove_stop_words=False, use_stemming=True)
