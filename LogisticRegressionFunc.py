...
df['tweets'] = df['tweets'].apply(lambda x: preprocess_text(x, use_lemmatization=True, remove_stop_words=True))
#print(df.head)
...
def logisticRegressionValue(string):
    if pd.isna(string):
        string = 'NaN'
    preprocess_text(string,use_lemmatization=True, remove_stop_words=True)
